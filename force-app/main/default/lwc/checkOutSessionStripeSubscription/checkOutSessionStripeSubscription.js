...
export default class checkOutSessionStripeSubscription extends LightningElement {
    ...
    async subscriptionFunction(packageIdParam){
        ...
        try {
            if (this.publisherKey && this.sessionId) {
                console.log('if block executed for opening stripe.....');
                const stripe = await Stripe(this.publisherKey);//pass publisher key
                console.log('stripe: ', stripe);
                const result = await stripe.redirectToCheckout({ sessionId: this.sessionId }); //pass sessionId
                console.log('stripe result>>>>>>>>>>>>: ', result);
                if (result.error) {
                    console.log('>>>>>>>>Error in payment: ', result.error.message);
                }
                else {
            ...
            console.log('Catch block executed leads to failure for opening stripe: ', error.message);
        }
    }
